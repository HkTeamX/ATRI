...
export class ATRI {
  ...
  //     if (!('debug' in config.bot))
  //       config.bot.debug = config.debug
  //     if (!('logLevel' in config.bot))
  //       config.bot.logLevel = config.logLevel
  ...
  //     }
  //     if (options.baseDir)
  //       options.import = createRequire(options.baseDir)
  //     if (!options.quiet)
  //       this.logger.INFO(`加载插件: ${packageName}`)

  //     if (this.loadedPlugins[packageName]) {
  //       if (!options.quiet)
  //         this.logger.WARN(`插件 ${packageName} 已加载，跳过加载`)
  ...
  //       catch (error) {
  //         if (!options.quiet)
  //           this.logger.ERROR(`插件 ${packageName} 导入失败:`, error)
  ...
  //     if (!module.Plugin) {
  //       if (!options.quiet)
  //         this.logger.ERROR(`插件 ${packageName} 加载失败: 未找到 Plugin 类`)
  ...
  //     catch (error) {
  //       if (!options.quiet)
  //         this.logger.ERROR(`插件 ${packageName} 实例化失败:`, error)
  //       return [1, error instanceof Error ? error.message : String(error)]
  //     }
  ...
  //       if (!options.quiet)
  //         this.logger.INFO(`插件 ${packageName} 加载成功`)
