...
export class Bot extends InjectLogger {
    ...
    return new Promise<Bot>((resolve, reject) => {
      ...
      ws.on('socket.error', (context) => {
        ...
        if (context.reconnection.nowAttempts >= context.reconnection.attempts) {
          reject(new Error(`重试次数超过设置的${context.reconnection.attempts}次!`))
  ...
  parseArgs(str: string): string[] {
    const matches = str.matchAll(this.argsRegex)
    const result: string[] = []
  ...
  expectedArgsRegex = /Expected (\d+) arguments? but got (\d+)\./
  ...
  getCommandHelpInformation(commandName: string) {
    ...
    if (!foundEvent || !foundEvent.commander)
      return undefined
